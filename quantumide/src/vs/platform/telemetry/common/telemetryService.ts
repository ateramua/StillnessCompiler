...
function telemetryDataCollectionDocUrl(): string {
	if (product.documentationUrl) {
		return product.documentationUrl.replace(/#.*$/, '') + '#telemetry-and-diagnostics';
	}
	return 'https://code.visualstudio.com/docs/getstarted/telemetry';
}
...
function getTelemetryLevelSettingDescription(): string {
	...
	const externalLinksStatement = !product.privacyStatementUrl ?
		localize("telemetry.docsStatement", "Read more about the [data we collect]({0}).", telemetryDataCollectionDocUrl()) :
		localize("telemetry.docsAndPrivacyStatement", "Read more about the [data we collect]({0}) and our [privacy statement]({1}).", telemetryDataCollectionDocUrl(), product.privacyStatementUrl);
