	...
	private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<void> {
		...
		const learnMoreButton: IPromptButton<void> = {
			...
			run: () => {
				...
				this.instantiationService.invokeFunction(accessor => accessor.get(ICommandService).executeCommand('vscode.open', URI.parse('https://code.visualstudio.com/docs/configure/extensions/extension-runtime-security')));
				throw new CancellationError();
			}
		};
		...
		const unverifiedLink = 'https://code.visualstudio.com/docs/configure/extensions/extension-runtime-security#_verified-publisher';
