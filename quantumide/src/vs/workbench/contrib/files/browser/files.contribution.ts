...
const hotExitConfiguration: IConfigurationPropertySchema = isNative ?
	{
		...
		'markdownDescription': nls.localize('hotExit', "[Hot Exit]({0}) controls whether unsaved files are remembered between sessions, allowing the save prompt when exiting the editor to be skipped.", 'https://code.visualstudio.com/docs/editor/codebasics#_hot-exit')
	} : {
		'type': 'string',
		'scope': ConfigurationScope.APPLICATION,
...
configurationRegistry.registerConfiguration({
	...
	'properties': {
		...
		[FILES_ASSOCIATIONS_CONFIG]: {
			'type': 'object',
			'markdownDescription': nls.localize('associations', "Configure [glob patterns](https://code.visualstudio.com/docs/editor/codebasics#_advanced-search-options) of file associations to languages (for example `\"*.extension\": \"html\"`). Patterns will match on the absolute path of a file if they contain a path separator and will match on the name of the file otherwise. These have precedence over the default associations of the languages installed."),
			'additionalProperties': {
				'type': 'string'
			}
		},
