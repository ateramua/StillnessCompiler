...
export class WorkspaceTrustEditor extends EditorPane {
	...
	private async render() {
		...
		const workspaceTrustDocUrl = this.productService.documentationUrl
			? this.productService.documentationUrl.replace(/#.*$/, '') + '#workspace-trust-and-restricted-mode'
			: 'https://code.visualstudio.com/docs/editor/workspace-trust';
		const headerDescriptionActionsText = localize({ key: 'workspaceTrustEditorHeaderActions', comment: ['Please ensure the markdown link syntax is not broken up with whitespace [text block](link block)'] }, "[Configure your settings]({0}) or [learn more]({1}).", `command:workbench.trust.configure`, workspaceTrustDocUrl);
