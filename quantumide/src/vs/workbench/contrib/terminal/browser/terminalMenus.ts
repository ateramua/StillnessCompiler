...
function isAiContributedProfile(profile: IExtensionTerminalProfile): boolean {
	...
	if (extensionIdentifier === 'github.copilot-chat' || extensionIdentifier === 'quantumide.personal-chat' || extensionIdentifier === 'anthropic.claude-code') {
		return true;
	}
...
function isAiProfileName(name: string): boolean {
	...
	return lowerCaseName.includes('copilot') || lowerCaseName.includes('claude') || lowerCaseName.includes('quantumide');
}
