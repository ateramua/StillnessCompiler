...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement): void {
		...
		if (renderInputOnTop) {
			...
			this.listContainer.setAttribute('data-testid', 'chat-list');
		} else {
			this.listContainer = dom.append(this.container, $(`.interactive-list`));
		...
		if (this.isLockedToCodingAgent) {
			...
			const message = providerMessage
				...
				: (this._lockedAgent?.prefix === '@copilot '
					? new MarkdownString(localize('copilotCodingAgentMessage', "This chat session will be forwarded to the {0} [coding agent]({1}) where work is completed in the background. ", this._lockedAgent.prefix, 'https://code.visualstudio.com/docs/copilot/agents/agents-window') + DISCLAIMER, { isTrusted: true })
