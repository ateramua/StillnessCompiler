		...
		if (this.#html !== value) {
			...
			if (this.#shouldRewriteOldResourceUris && !this.#hasCalledAsWebviewUri && /(["'])vscode-resource:([^\s'"]+?)(["'])/i.test(value)) {
				...
				this.#deprecationService.report('Webview vscode-resource: uris', this.#extension,
					`Please migrate to use the 'webview.asWebviewUri' api instead: https://code.visualstudio.com/api/extension-guides/webview#_loading-local-content`);
			}
...
export class ExtHostWebviews extends Disposable implements extHostProtocol.ExtHostWebviewsShape {
	...
	public $onMissingCsp(
		...
		this._logService.warn(`${extensionId} created a webview without a content security policy: https://code.visualstudio.com/api/extension-guides/webview#_content-security-policy`);
	}
